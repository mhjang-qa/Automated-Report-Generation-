...
노션 티켓 링크를 입력하면 티켓 내용을 읽고 Gemini로 작업 내용을 요약한 뒤, 테스트 정보와 함께 지정된 노션 DB에 저장합니다. 저장된 요약을 기반으로 테스트 케이스 Markdown Table을 생성하고 같은 노션 페이지에 좌/우 2단 컬럼 형태로 업로드할 수 있습니다.
...
NOTION_COMMENT_LIMIT=30
NOTION_COMMENT_BLOCK_SCAN_LIMIT=80
NOTION_COMMENTS_VERSION=2026-03-11
...
- `NOTION_COMMENT_LIMIT`: 요약 생성 시 함께 참고할 노션 댓글 최대 개수, 기본값 `30`
- `NOTION_COMMENT_BLOCK_SCAN_LIMIT`: 블록 댓글 확인을 위해 스캔할 하위 블록 최대 개수, 기본값 `80`
- `NOTION_COMMENTS_VERSION`: Notion Comments API 호출 버전, 기본값 `2026-03-11`
...
3. 댓글까지 분석하려면 Notion Developer Portal에서 해당 Integration의 comment read capability를 켭니다.
4. 대상 DB URL은 기본 요구사항의 DB URL을 사용합니다. 다른 DB를 쓰려면 `NOTION_TARGET_DB_URL` 또는 `NOTION_TARGET_DATABASE_ID`를 설정합니다.
5. DB가 비어 있어도 괜찮습니다. 앱이 신규 row를 만들고 필요한 속성(`원본 노션 링크`, `요약 상태`, `생성 일시`, `TC 생성 여부`)을 자동 추가합니다.
...
6. `분석 요약`을 눌러 티켓 본문, 이미지 블록, 최신 댓글/논의 조회와 Gemini 요약 생성을 실행합니다.
7. 결과를 확인한 뒤 `노션 등록`을 눌러 대상 DB에 `(초안 분석)` 제목 suffix가 붙은 신규 페이지를 생성합니다. 본문에는 `작업 내용 요약` 아래 `Test info.` / `테스트 정보` 콜아웃이 함께 저장됩니다.
8. `TC 생성하기`를 눌러 Markdown Table 형식의 테스트 케이스를 생성합니다.
9. `TC 업로드`를 눌러 저장된 노션 페이지를 왼쪽 `작업 내용 요약`, 오른쪽 `테스트 케이스 - 초안` 표 형태의 2단 컬럼으로 정리합니다.
...
4. `비교 시작`을 누르면 서버가 Figma API로 Frame PNG를 생성하고, 비교 Canvas에 `Figma 기준`, `실제 화면`, `Overlay 비교` 영역을 분리해 표시합니다.
5. 기본적으로 Figma의 실제 단말 유사 영역인 상단 52px, 하단 34px은 diff 제외 영역으로 표시하고, 중앙 콘텐츠 영역만 비교 대상으로 봅니다.
6. Opacity, X/Y offset, Scale 값은 `Overlay 비교` 영역에 적용해 UI 차이를 검수합니다.

`X-Frame-Options: SAMEORIGIN` 또는 `Content-Security-Policy: frame-ancestors`가 감지되면 PixelAudit은 제한적 서버 프록시(`/api/pixel/proxy`)로 전환해 미리보기를 시도합니다. 프록시는 public URL만 허용하며 localhost, loopback, private IP, link-local 주소는 차단합니다. Playwright 기반 서버 캡처와 픽셀 단위 Difference 계산은 포함하지 않았고, 필요하면 Render 빌드에 `playwright` 브라우저 설치 단계를 추가하는 별도 Phase로 확장합니다.

`Web AVD 360` 버튼은 실제 웹 URL을 360 × 800 가상 단말 런처(`/pixel-device.html`)로 엽니다. 별도 창에서 모바일 웹 기준 표시 상태를 확인할 수 있고, iframe 차단 헤더가 있으면 PixelAudit 프록시로 자동 전환합니다.

## GoHanpass CDN 다국어 검증

상단 `다국어 검증` 탭은 별도 프로젝트인 `go_hanpass_localization_validator` 웹앱을 iframe으로 함께 띄웁니다.

로컬에서 같이 실행하려면 별도 터미널에서 다국어 검증 앱을 먼저 실행합니다.

```bash
cd "/Users/jangminho/Desktop/One_click_0519/go_hanpass_localization_validator/backend"
source .venv/bin/activate
uvicorn app.main:app --reload
```

```bash
cd "/Users/jangminho/Desktop/One_click_0519/go_hanpass_localization_validator/frontend"
npm run dev
```
...
- 노션 댓글은 `[댓글/논의]` 섹션으로 본문에 합쳐 Gemini에 전달하며, 댓글 첨부 이미지도 제한 개수/용량 안에서 함께 전달합니다.
- 저장 페이지 본문은 Notion `column_list`를 사용해 요약은 왼쪽, TC 표는 오른쪽 2단 컬럼으로 구성합니다.
- 테스트 정보는 제목/요약/원문 키워드를 기반으로 테스트 명칭, 테스트 유형, 대상 환경, 플랫폼, 검증 방식, 검증 대상을 자동 정리합니다.
